        ...
        <td width="83%" valign="top">
          ...
          <tr>
            <td height="333" valign="top"><?php
				...
				if($senha!=$senha1){
                   echo "<p>As senhas devem ser iguais";
                exit;
				}
				//liga a BD
				include("ligarBD.php");
				...
				$existe="SELECT * FROM cliente WHERE email='".$email."'OR utilizador='".$utilizador."'";
				$faz_existe=mysqli_query($ligaBD,$existe);
				$jaexiste=mysqli_num_rows($faz_existe);

				if($jaexiste==0){
				$insere_utilizador="INSERT into cliente (NIF, nome, email, utilizador, senha) VALUES ('".$NIF."', '".$nome."', '".$email."', '".$utilizador."','".$senha."')";
				$faz_insere_utilizador=mysqli_query($ligaBD,$insere_utilizador);
				echo "<p>O utilizador foi registado com sucesso!";
				}
				else
				echo "<p>O utilizador já se encontra registado!";
				?>                         </td>
          </tr>
        </table>          </td>
      </tr>
